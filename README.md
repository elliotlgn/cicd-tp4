...
### 🎯 Objectif
Automatiser la génération de rapports Allure et le monitoring dans le pipeline CI/CD.

**Ce que vous allez faire :**
- Créer un workflow GitHub Actions qui exécute les tests
- Générer le rapport Allure HTML
- Publier le rapport comme artifact
- Optionnellement, exporter les métriques vers un Prometheus hébergé

**Pourquoi :** L'automatisation CI/CD permet d'avoir des rapports à jour à chaque commit et de suivre l'évolution des tests.

### 🧩 Instructions

#### 5.1 Créer le workflow GitHub Actions
  ...
  test-and-report:
    ...
    steps:
      ...
      - name: 💬 Comment PR with test results
        ...
        with:
          script: |
            ...
            // Lire les résultats Allure (simplifié)
            let testSummary = "✅ Tests exécutés avec Allure\n\n";
            testSummary += "📊 [Télécharger le rapport HTML complet](https://github.com/${{ github.repository }}/actions/runs/${{ github.run_id }})\n";
      ...
      - name: ❌ Fail if tests failed
        if: failure()
        run: |
          echo "❌ Certains tests ont échoué. Consultez le rapport Allure pour plus de détails."
          exit 1
```

#### 5.2 Créer le .gitignore
...
# Créer le workflow
# (Copier le contenu ci-dessus dans .github/workflows/ci-monitoring.yml)

# Commit et push
git add .
git commit -m "feat: Add CI/CD with Allure reporting"
...
✅ Tests exécutés automatiquement sur chaque push  
✅ Rapport Allure HTML généré et publié comme artifact  
✅ Rapport téléchargeable depuis l'onglet Actions de GitHub